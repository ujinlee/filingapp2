...
def extract_financials_from_sec_json(sec_json):
    forms = sec_json['filings']['recent']
    periods = []
    revenue = []
    net_income = []
    eps_basic = []
    print("Available us-gaap facts:", list(sec_json['facts']['us-gaap'].keys()))
                        ...
                        for unit in fact['units'].values():
                            for entry in unit:
                                ...
                                try:
                                    ...
                                    continue
                    return None
                revenue_val = get_fact_value(rev, period) if rev else None
                net_income_val = get_fact_value(ni, period) if ni else None
                eps_val = get_fact_value(eps, period) if eps else None
                # Print debug info for each value
                print(f"Period: {period} | Revenue: {revenue_val} | Net Income: {net_income_val} | EPS: {eps_val}")
                revenue.append(revenue_val)
                net_income.append(net_income_val)
                eps_basic.append(eps_val)
            except Exception as e:
