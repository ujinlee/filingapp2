...
def extract_key_financials_from_sec_json(sec_json, num_periods=2):
    ...
    - Uses fallback us-gaap labels for each metric.
    - Allows off-by-one day for date matching.
    - Only uses numbers present in the JSON, never invents or estimates.
    ...
    # Helper to get value for a given fact and period (with off-by-one day matching)
    ...
    # Fallback label lists for each metric
    revenue_labels = [
        'Revenues',
    ...
    for i, form in enumerate(forms['form']):
        ...
        try:
            ...
            # Find first available value for each metric using fallback labels
            rev = None
            ...
            for label in eps_labels:
                ...
                if eps:
                    break
            revenue.append(get_fact_value(rev, period))
            net_income.append(get_fact_value(ni, period))
            eps_basic.append(get_fact_value(eps, period))
        except Exception as e:
    ...
    # Print only the final extracted values for each period
...
class SummarizationAgent:
    ...
    def summarize(content: str, allowed_numbers=None) -> str:
        ...
        if allowed_numbers is not None:
            summary, flagged = check_script_numbers(summary, allowed_numbers)
            if flagged:
