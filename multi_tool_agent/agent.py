...
def extract_financials_from_sec_json(sec_json):
    # Find the most recent 10-Q or 10-K filing
    forms = sec_json['filings']['recent']
    periods = []
    revenue = []
    net_income = []
    eps_basic = []
    # Print available us-gaap fact keys for debugging
    print("Available us-gaap facts:", list(sec_json['facts']['us-gaap'].keys()))
    # Try to get up to 2 periods for comparison
    for i, form in enumerate(forms['form']):
        if form in ['10-Q', '10-K']:
            ...
            try:
                ...
                # Try common alternative labels for revenue
                rev = (
                ...
                # Try common alternative labels for net income
                ni = (
                ...
                # Try common alternative labels for EPS
                eps = (
                        ...
                        for unit in fact['units'].values():
                            for entry in unit:
                                if entry['end'][:10] == period:
                                    return entry['val']
                    return None
                revenue.append(get_fact_value(rev) if rev else None)
                net_income.append(get_fact_value(ni) if ni else None)
                eps_basic.append(get_fact_value(eps) if eps else None)
            except Exception as e:
