...
def extract_key_financials_from_sec_json(sec_json, num_periods=2):
    ...
    periods, revenue, net_income, eps_basic = [], [], [], []
    # Try to get up to num_periods for comparison
    for i, form in enumerate(forms['form']):
        if form in ['10-Q', '10-K']:
            ...
            try:
                ...
                # Try common alternative labels for each metric
                rev = (facts.get('Revenues') or
                       facts.get('RevenueFromContractWithCustomerExcludingAssessedTax') or
                       facts.get('SalesRevenueNet') or
                       facts.get('SalesRevenueGoodsNet') or
                       facts.get('TotalRevenuesAndOtherIncome') or {})
                ni = (facts.get('NetIncomeLoss') or
                      facts.get('ProfitLoss') or
                      facts.get('NetIncomeLossAvailableToCommonStockholdersBasic') or {})
                eps = (facts.get('EarningsPerShareBasic') or
                       facts.get('EarningsPerShareBasicAndDiluted') or {})
                def get_fact_value(fact, period):
                    if 'units' in fact:
                        for unit in fact['units'].values():
                            for entry in unit:
                                # Try exact match
                                if entry['end'][:10] == period:
                                    return entry['val']
                                # Try off-by-one (sometimes SEC filings are off by a day)
                                try:
                                    entry_date = datetime.strptime(entry['end'][:10], "%Y-%m-%d")
                                    period_date = datetime.strptime(period, "%Y-%m-%d")
                                    if abs((entry_date - period_date).days) <= 1:
                                        return entry['val']
                                except Exception:
                                    continue
                    return None
                revenue.append(get_fact_value(rev, period) if rev else None)
                net_income.append(get_fact_value(ni, period) if ni else None)
                eps_basic.append(get_fact_value(eps, period) if eps else None)
            except Exception as e:
                revenue.append(None)
                net_income.append(None)
                eps_basic.append(None)
            if len(periods) == num_periods:
                break
    # Print only the final extracted values
    for i in range(len(periods)):
        print(f"Period: {periods[i]} | Revenue: {revenue[i]} | Net Income: {net_income[i]} | EPS: {eps_basic[i]}")
    return periods, revenue, net_income, eps_basic

def build_podcast_prompt_from_financials(periods, revenue, net_income, eps_basic):
    def calc_growth(new, old):
        try:
            return (float(new) - float(old)) / float(old) * 100
        except:
            return None
    prompt_lines = ["Here are the key financial numbers for the company:"]
    for i in range(len(periods)):
        prompt_lines.append(f"- Period: {periods[i]}")
        prompt_lines.append(f"  Revenue: ${revenue[i]} million")
        prompt_lines.append(f"  Net income: ${net_income[i]} million")
        prompt_lines.append(f"  Basic earnings per share: ${eps_basic[i]}")
        if i > 0 and revenue[i] and revenue[i-1]:
            growth = calc_growth(revenue[i], revenue[i-1])
            if growth is not None:
                prompt_lines.append(f"  Revenue YoY/QoQ growth: {growth:.1f}% (from ${revenue[i-1]} million)")
        if i > 0 and net_income[i] and net_income[i-1]:
            growth = calc_growth(net_income[i], net_income[i-1])
            if growth is not None:
                prompt_lines.append(f"  Net income YoY/QoQ growth: {growth:.1f}% (from ${net_income[i-1]} million)")
        if i > 0 and eps_basic[i] and eps_basic[i-1]:
            growth = calc_growth(eps_basic[i], eps_basic[i-1])
            if growth is not None:
                prompt_lines.append(f"  EPS YoY/QoQ growth: {growth:.1f}% (from ${eps_basic[i-1]})")
    prompt_lines.append("\nYou must only use the numbers provided above. Do not use or mention any other numbers. If you need to reference a number, it must be one of the numbers listed above. If a number is not provided, do not estimate, invent, or mention it.")
    prompt_lines.append("Please generate a 3-4 minute podcast script as a conversation between Alex and Jamie, focusing on these numbers and their significance. If you discuss reasons for changes, use only what is explicitly stated in the provided context.")
    return '\n'.join(prompt_lines)
...
def check_script_numbers(script, allowed_numbers):
    # Find all numbers in the script (integers, floats, and currency)
    found_numbers = set(re.findall(r'\$?\d+[\d,.]*', script))
    allowed_set = set(str(n) for n in allowed_numbers if n is not None)
    flagged = False
    for num in found_numbers:
        num_clean = num.replace('$', '').replace(',', '')
        if num_clean not in allowed_set:
            # Replace all occurrences of this number with [REDACTED]
            script = re.sub(rf'\b{re.escape(num)}\b', '[REDACTED]', script)
            flagged = True
    if flagged:
        script += "\n[WARNING: Some numbers were replaced with [REDACTED] because they are not in the allowed set!]"
    return script, flagged

# Example usage after LLM call:
# allowed_numbers = list(revenue) + list(net_income) + list(eps_basic)
# script, flagged = check_script_numbers(script, allowed_numbers)
# if flagged:
