...
async def summarize_filing(request: SummarizeRequest):
    ...
    try:
        ...
        prompt = (
            ...
            "THOROUGHLY review ONLY the section above for relevant explanations and drivers. For revenue, always mention the explicit drivers if stated. For net income changes, specifically look for and mention changes in cost of revenue, operating expenses, or other major cost items if discussed. "
            "IMPORTANT: Do NOT mention or refer to the Management's Discussion and Analysis section, 'MDA', or any similar phrase in any language. Only state the facts and drivers as described in the section above. "
            "IMPORTANT: If a number (such as revenue) is not available, simply omit it from the script. Do NOT mention or allude to its absence in any way. "
            "When listing drivers for changes in costs or revenue, you must mention all major drivers as stated in the section above, in the order and with the relative importance or amounts as described. Do NOT emphasize or highlight a single factor unless the section above does so. Do not mislead by suggesting a minor factor is the main driver. "
            "If the section above discusses different business segments, report the changes and drivers for each segment as stated, without blending or generalizing. If a segment increased, state the increase and the reason. If a segment decreased, state the decrease and the reason. Do not mix up increases and decreases between segments. "
            "If no reason is given, simply state the change (e.g., 'Net income increased this quarter.') and do not speculate, hedge, or mention the absence of details. "
            ...
            "Do NOT mention or refer to the Management's Discussion and Analysis section, 'MDA', or management commentary by name or description. Just incorporate the insights naturally, as if you are discussing the company's performance and outlook. "
