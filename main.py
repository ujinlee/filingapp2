...
async def summarize_filing(request: SummarizeRequest):
    ...
    try:
        ...
        def clean_transcript(transcript):
            ...
            transcript = re.sub(r"^(ALEX:)\s*(Hey, everyone, )?(this is|I am|I'm)?\s*Alex[,.!\s-]*", r"\1 ", transcript, flags=re.MULTILINE | re.IGNORECASE)
            transcript = re.sub(r"^(JAMIE:)\s*(Hey, everyone, )?(this is|I am|I'm)?\s*Jamie[,.!\s-]*", r"\1 ", transcript, flags=re.MULTILINE | re.IGNORECASE)
